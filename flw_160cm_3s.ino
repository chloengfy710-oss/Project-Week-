...
// First 160 cm move
bool initialMoveDone = false;
bool pauseDone = false; // stop only once after 160 cm
...
void loop() {
  ...
  float distance = calculatedistance(); // get distance traveled

  // --- Stop once after 160 cm ---
  if (!initialMoveDone && distance >= 165.0 && !pauseDone) {
    stop();
    delay(3000);          // pause 3 seconds
    leftencodercount = 0; // reset encoders
    rightencodercount = 0;
    pauseDone = true;
  ...
  if (L && R) {
    ...
    turnLeft();       // keep delay for sharp turns
  }
  else if (L && !R) {
    turnRight();
    delay (300);     // keep delay for sharp turns
  }
  else {
    stop();
    while (true){};   // stop forever if both sensors see black
  } 
}
